...
class XceptionTime_model(nn.Module):
    def __init__(self, in_channels, out_channels, frames, node_num):
        ...
        self.conv_s1 = nn.Conv2d(out_channels, out_channels, kernel_size=5, padding=5//2)
        self.conv_s2 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=7, padding=7//2),
            nn.Conv2d(out_channels, out_channels, kernel_size=7, padding=7//2),
        )
...
class Double_mstff(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # y = mish(y + res)
        # # 预解码
        # y = self.decode(y)  # N,64

        return y


if __name__ == '__main__':
    x1 = torch.rand(50, 1, 32, 19)
    mo = Double_mstff(1, 18, 32, 16)
    print(mo(x1).shape)